...
from basicsr.utils.settings import TRAIN_IMG_H, TRAIN_IMG_W, TEST_IMG_H, TEST_IMG_W
from basicsr.utils.img_util import rgb_to_bgr
...
class DataAugmenter:
    ...
    @staticmethod
    def __random_flip(img: np.ndarray, numpy_gt: np.ndarray) -> tuple:
        """ Perform random left/right flip with probability 0.5 """
        if random.randint(0, 1):
            img = img[:, ::-1]
            numpy_gt = numpy_gt[:, ::-1]
        return img.astype(np.float32), numpy_gt.astype(np.float32)
    ...
    def augment(self, img: np.ndarray, numpy_gt: np.ndarray) -> tuple:
        scale = math.exp(random.random() * math.log(self.__scale[1] / self.__scale[0])) * self.__scale[0]
        s = min(max(int(round(min(img.shape[:2]) * scale)), 10), min(img.shape[:2]))
        ...
        start_y = random.randrange(0, img.shape[1] - s + 1)
        print(img.shape)
        print(numpy_gt.shape)
        img = img[start_x:start_x + s, start_y:start_y + s]
        numpy_gt = numpy_gt[start_x:start_x + s, start_y:start_y + s]
        print(img.shape)
        print(numpy_gt.shape)
        rand = random.random()
        img = self.__rotate_and_crop(img, angle=(rand - 0.5) * self.__angle)
        numpy_gt = self.__rotate_and_crop(numpy_gt, angle=(rand - 0.5) * self.__angle)
        img, numpy_gt = self.__random_flip(img, numpy_gt)

        color_aug = np.zeros(shape=(3, 3))
        for i in range(3):
            color_aug[i, i] = 1 + random.random() * self.__color - 0.5 * self.__color
        img *= np.array([[[color_aug[0][0], color_aug[1][1], color_aug[2][2]]]], dtype=np.float32)
        new_numpy_gt *= np.array([[[color_aug[0][0], color_aug[1][1], color_aug[2][2]]]], dtype=np.float32)
        new_image = np.clip(img, 0, 65535)
        new_numpy_gt = np.clip(numpy_gt, 0, 65535)

        return new_image, new_numpy_gt
